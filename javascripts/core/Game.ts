...
export class MinesweeperGame extends MinesweeperGameBase {
    ...
    _onGenerationWarning(result: { cancelled?: boolean; warning?: boolean }): void {
        ...
        if (typeof window !== 'undefined' && (window as any)._gameController?.uiManager?.hudController) {
            (window as any)._gameController.uiManager.hudController.showNotification(msg, 'warning');
        }
